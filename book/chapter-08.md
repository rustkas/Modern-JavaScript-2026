...
- **CSSOM** — объектная модель CSS. JavaScript может управлять стилями через `element.style`, `CSSStyleSheet` и **CSS Typed OM**. Здесь важна оговорка: `element.style` и `CSSStyleSheet` — давно устоявшиеся, полностью кросс-браузерные механизмы, а вот CSS Typed OM (типизированные представления CSS-значений вместо строк) на момент написания книги **не входит в Baseline** — технология не поддерживается во всех основных браузерах и остаётся нишевым инструментом для узкого круга задач, требующих повышенной производительности при частых манипуляциях со стилями. Для большинства проектов достаточно `element.style` и CSS-классов/переменных, о которых говорится ниже.
