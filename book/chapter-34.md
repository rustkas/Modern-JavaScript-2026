...
Здесь важно повторить оговорку, уже сделанную в главе об архитектуре рантайма и главе о многопоточности: *Paint Worklet* и особенно *Animation Worklet* нельзя описывать как готовую, равномерно доступную технологию. Paint Worklet реализован в Chromium-браузерах и частично в Safari (с версии 16.4), но в стабильном Firefox доступен только за флагом — статус Baseline не достигнут. Animation Worklet зашёл ещё меньше: поддержка фактически ограничена Chromium. Для многих сценариев, ради которых был создан Animation Worklet, современные браузеры уже предлагают CSS Scroll-Driven Animations как более зрелую декларативную альтернативу.

Поэтому корректная формулировка звучит так: там, где Paint Worklet и Animation Worklet действительно доступны, изощрённая рендер-логика может исполняться в изолированных вспомогательных потоках, что способствует стабильности кадровой частоты. Но использовать эти технологии как единственный механизм отрисовки важного элемента интерфейса в продакшене, ориентированном на все основные браузеры, преждевременно — необходим фоллбэк.
...
```css
/* Использование Paint Worklet — с фоллбэком через @supports */
.checkerboard {
  background-image: repeating-conic-gradient(red 0% 25%, blue 0% 50%); /* fallback */
  width: 200px;
  height: 200px;
}

@supports (background-image: paint(checkerboard)) {
  .checkerboard {
...
Принцип непрерывной стандартизации *Baseline* обеспечил кросс-браузерную предсказуемость для многих архитектурных примитивов, однако точные даты важно указывать корректно — некоторые технологии достигли этого статуса значительно позже, чем можно предположить по их раннему появлению в отдельных браузерах:

| Технология | Статус Baseline | Год/дата включения |
| ---------- | --------------- | ------------- |
...
| **WebGPU** | **Newly available** | **Ноябрь 2025** (несмотря на поддержку в Chrome с 2023 года — Safari и Firefox на Windows добавили поддержку только во второй половине 2025 года) |
| **View Transitions (same-document)** | **Newly available** | **2024** — переходы в пределах одного документа (для SPA) |
| **View Transitions (cross-document)** | **Не входит в Baseline** | Поддерживается в Chromium и Safari, но не в стабильном Firefox |

Отдельно стоит подчеркнуть: View Transitions — не единая по зрелости технология. Переходы в пределах одного документа (same-document, актуальные для SPA) действительно достигли статуса Newly Available, тогда как переходы между разными страницами (cross-document, для многостраничных приложений) на момент написания книги ещё не входят в Baseline из-за отсутствия поддержки в стабильном Firefox.
