...
Все специальные символы автоматически кодируются согласно правилам `application/x-www-form-urlencoded`, которые для `URLSearchParams` отличаются от общего percent-encoding, используемого, например, для пути URL:
...
- Пробел **всегда** кодируется как `+` — в отличие от пути или фрагмента URL, где пробел кодируется как `%20`, у `URLSearchParams` строго фиксированное поведение по спецификации, без альтернативных вариантов.
...
**Важная асимметрия, о которой стоит знать:** поскольку пробел кодируется именно как `+`, при *разборе* строки `URLSearchParams` симметрично интерпретирует символ `+` как пробел. Это может привести к скрытым ошибкам при работе с данными, где `+` несёт иной смысл — например, с Base64-строками:

```javascript
// Base64 может содержать "+" как значащий символ
const params = new URLSearchParams("bin=E+AXQB+A");
console.log(params.get("bin")); // "E AXQB A" — "+" ошибочно превратился в пробел

// Решение — предварительно закодировать значение
params.append("bin", encodeURIComponent("E+AXQB+A"));
```

Также стоит иметь в виду ещё одно расхождение между `URL` и `URLSearchParams`: тильда (`~`) не кодируется в `url.search`, но `URLSearchParams` кодирует её всегда — при смешанном использовании `url.search` и `url.searchParams` в одном проекте это может привести к несогласованным результатам сериализации одного и того же значения.
