...
`Object.hasOwn()` стандартизирован в **ES2022** и имеет статус **Baseline Widely Available** — поддерживается во всех основных браузерах уже более 30 месяцев, поэтому его можно использовать без каких-либо оговорок в любом проекте.
...
`Object.groupBy()` и `Map.groupBy()` стандартизированы в **ES2024** (Baseline Newly Available с марта 2024 года) и работают во всех основных браузерах, включая Safari с версии 17.4, а также в Node.js (с версии 21) и Deno. Статус **Baseline Widely Available** для этих методов ожидается ориентировочно к сентябрю 2026 года — то есть де-факто они уже безопасны для использования без полифилов в большинстве проектов, но формально ещё не пересекли порог в 30 месяцев.
...
Для ориентира: `Object.fromEntries()` и `Object.entries()` — давно устоявшиеся методы **ES2019/ES2017** соответственно, входящие в Baseline Widely Available без каких-либо ограничений. Таким образом, весь набор методов этого раздела делится на две группы по зрелости: полностью «безопасные» (`hasOwn`, `entries`, `fromEntries`) и относительно новые, но уже кросс-браузерные (`groupBy`-методы), для которых формальный статус Widely Available наступит в течение 2026 года.
