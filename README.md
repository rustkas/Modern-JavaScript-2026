# Modern JavaScript 2026

## Современный JavaScript в эпоху Web Platform, AI и Baseline
> От языка сценариев к архитектуре современной веб-платформы.

---

## 📖 О книге

За последние годы JavaScript проделал огромный путь.

Сегодня это не просто язык для анимации кнопок — это фундаментальная технология Web Platform, лежащая в основе современных веб-приложений, сложных пользовательских интерфейсов и даже приложений с искусственным интеллектом.

В книге рассматривается JavaScript 2026 года:

- новый синтаксис и возможности ECMAScript;
- работа с DOM и браузерными API;
- асинхронность и потоки данных;
- современные архитектурные подходы;
- интеграция с WebAssembly и AI;
- обзор актуальных фреймворков и их будущего.

Современные фреймворки:

- React
- Angular
- Vue
- Svelte
- Qwik
- Astro

не заменяют JavaScript.  
Они строятся **поверх JavaScript**, используя язык и веб-платформу как основу.

Эта книга показывает JavaScript как часть современной Web Platform.

---

## 🎯 Цель книги

После изучения книги читатель должен понимать:

- как устроен современный JavaScript и его экосистема;
- как браузер выполняет JavaScript;
- как работать с современными API браузера;
- как строить масштабируемые приложения на чистом JavaScript;
- как использовать асинхронность и потоки;
- как оптимизировать производительность приложений;
- как интегрировать AI и WebAssembly;
- какую роль играют фреймворки в 2026 году.

---

## 👥 Для кого эта книга

Книга предназначена для:

- Frontend-разработчиков всех уровней;
- разработчиков, использующих React, Angular, Vue, Svelte;
- архитекторов интерфейсов и технических лидов;
- всех, кто хочет углубить понимание JavaScript и Web Platform;
- специалистов по производительности и оптимизации;
- разработчиков, интересующихся AI в вебе.

---

## 🧠 Главная идея

Современный frontend строится на фундаменте Web Platform:

```
JavaScript
    |
    |-- логика и поведение

HTML
    |
    |-- структура и смысл

CSS
    |
    |-- внешний вид и layout
```

Фреймворки — это дополнительный уровень абстракции:

```
React / Angular / Vue / Svelte / Qwik / Astro
                    ↓
        JavaScript + HTML + CSS
                    ↓
        Browser Web Platform
```

В 2026 году JavaScript — это:

- **зрелый язык** с современным синтаксисом;
- **платформа** с сотнями встроенных API;
- **экосистема** с огромным количеством инструментов;
- **основа** для AI-приложений в браузере;
- **будущее** веб-разработки.

---

## 📚 Содержание

### Предисловие

## Почему JavaScript снова изменился

- От ES5 к ES2026
- TC39 и процесс развития языка
- Living Standard и непрерывная эволюция
- Baseline как новая модель совместимости
- Почему больше нет «старых браузеров»
- JavaScript как язык Web Platform

* [📖 Читать главу](./book/preface.md)  
* [📚 Литература](./references/preface.md)  
* [💻 Примеры](./examples/preface.md)  
* [🧪 Практика](./exercises/preface.md)

---

# Часть I. JavaScript в 2026 году

## Глава 1. Почему JavaScript снова изменился: Эволюция живой платформы

Темы:

- От скромных скриптов к системной архитектуре
- Механизм стандартизации: Комитет TC39 и пайплайн
- TC39 и процесс развития языка
- Living Standard и конец эпохи «больших версий»
- Модель совместимости Baseline и исчезновение «старых браузеров»
- JavaScript как единый язык Web Platform


* [📖 Читать главу](./book/chapter-01.md)  
* [📚 Литература](./references/chapter-01.md)  
* [💻 Примеры](./examples/chapter-01.md)  
* [🧪 Практика](./exercises/chapter-01.md)

---

## Глава 2. Архитектура современного JavaScript

Темы:

- Execution Context
- Lexical Environment
- Environment Records
- Call Stack
- Heap
- Garbage Collection
- Event Loop
- Task Queue
- Microtask Queue
- Rendering Pipeline

* [📖 Читать главу](./book/chapter-02.md)  
* [📚 Литература](./references/chapter-02.md)  
* [💻 Примеры](./examples/chapter-02.md)  
* [🧪 Практика](./exercises/chapter-02.md)

---

## Глава 3. JavaScript как декларативный язык платформы

Темы:

- HTML API
- CSS API
- Browser API
- DOM API
- URL API
- History API
- Navigation API
- Permissions API
- Clipboard API
- Почему JavaScript всё чаще становится «оркестратором», а не исполнителем

* [📖 Читать главу](./book/chapter-03.md)  
* [📚 Литература](./references/chapter-03.md)  
* [💻 Примеры](./examples/chapter-03.md)  
* [🧪 Практика](./exercises/chapter-03.md)

---

# Часть II. Современный язык JavaScript

## Глава 4. Современный синтаксис

Темы:

- let / const
- Destructuring
- Spread
- Rest
- Optional Chaining
- Nullish Coalescing
- Template Literals
- Tagged Templates
- Private Fields
- Static Blocks

* [📖 Читать главу](./book/chapter-04.md)  
* [📚 Литература](./references/chapter-04.md)  
* [💻 Примеры](./examples/chapter-04.md)  
* [🧪 Практика](./exercises/chapter-04.md)

---

## Глава 5. Функции нового поколения

Темы:

- Arrow Functions
- Closures
- Higher Order Functions
- Functional Programming
- Pure Functions
- Currying
- Partial Application
- Function Composition

* [📖 Читать главу](./book/chapter-05.md)  
* [📚 Литература](./references/chapter-05.md)  
* [💻 Примеры](./examples/chapter-05.md)  
* [🧪 Практика](./exercises/chapter-05.md)

---

## Глава 6. Асинхронность

Темы:

- Event Loop
- Promise
- async/await
- AbortController
- Async Iterators
- Async Generators
- Top-Level Await
- Почему callback hell окончательно ушёл в прошлое

* [📖 Читать главу](./book/chapter-06.md)  
* [📚 Литература](./references/chapter-06.md)  
* [💻 Примеры](./examples/chapter-06.md)  
* [🧪 Практика](./exercises/chapter-06.md)

---

## Глава 7. Модули ES Modules

Темы:

- import
- export
- dynamic import
- modulepreload
- Import Maps
- Tree Shaking
- Module Graph

* [📖 Читать главу](./book/chapter-07.md)  
* [📚 Литература](./references/chapter-07.md)  
* [💻 Примеры](./examples/chapter-07.md)  
* [🧪 Практика](./exercises/chapter-07.md)

---

# Часть III. Работа с браузером

## Глава 8. DOM как объектная модель документа

Темы:

- Document
- Element
- Node
- Text
- Shadow DOM
- MutationObserver

* [📖 Читать главу](./book/chapter-08.md)  
* [📚 Литература](./references/chapter-08.md)  
* [💻 Примеры](./examples/chapter-08.md)  
* [🧪 Практика](./exercises/chapter-08.md)

---

## Глава 9. Современные события

Темы:

- EventTarget
- addEventListener
- once
- passive
- signal
- AbortSignal
- Pointer Events
- Keyboard Events
- Input Events

* [📖 Читать главу](./book/chapter-09.md)  
* [📚 Литература](./references/chapter-09.md)  
* [💻 Примеры](./examples/chapter-09.md)  
* [🧪 Практика](./exercises/chapter-09.md)

---

## Глава 10. Custom Elements

Темы:

- Web Components
- Lifecycle
- observedAttributes
- Shadow DOM
- ElementInternals
- Form Associated Custom Elements

* [📖 Читать главу](./book/chapter-10.md)  
* [📚 Литература](./references/chapter-10.md)  
* [💻 Примеры](./examples/chapter-10.md)  
* [🧪 Практика](./exercises/chapter-10.md)

---

## Глава 11. Современные Browser APIs

Темы:

- Clipboard API
- Web Share API
- Web Locks
- BroadcastChannel
- Screen Wake Lock
- Fullscreen API
- File System Access API
- Storage API

* [📖 Читать главу](./book/chapter-11.md)  
* [📚 Литература](./references/chapter-11.md)  
* [💻 Примеры](./examples/chapter-11.md)  
* [🧪 Практика](./exercises/chapter-11.md)

---

# Часть IV. Современная работа с данными

## Глава 12. JSON сегодня

Темы:

- JSON
- Structured Clone
- structuredClone()
- Serialization
- Transferable Objects

* [📖 Читать главу](./book/chapter-12.md)  
* [📚 Литература](./references/chapter-12.md)  
* [💻 Примеры](./examples/chapter-12.md)  
* [🧪 Практика](./exercises/chapter-12.md)

---

## Глава 13. Fetch API

Темы:

- fetch()
- Request
- Response
- Headers
- Streams
- AbortController

* [📖 Читать главу](./book/chapter-13.md)  
* [📚 Литература](./references/chapter-13.md)  
* [💻 Примеры](./examples/chapter-13.md)  
* [🧪 Практика](./exercises/chapter-13.md)

---

## Глава 14. Streams API

Темы:

- ReadableStream
- WritableStream
- TransformStream
- Streaming HTML
- Streaming JSON

* [📖 Читать главу](./book/chapter-14.md)  
* [📚 Литература](./references/chapter-14.md)  
* [💻 Примеры](./examples/chapter-14.md)  
* [🧪 Практика](./exercises/chapter-14.md)

---

## Глава 15. URL API

Темы:

- URL
- URLSearchParams
- Blob URL
- Object URL

* [📖 Читать главу](./book/chapter-15.md)  
* [📚 Литература](./references/chapter-15.md)  
* [💻 Примеры](./examples/chapter-15.md)  
* [🧪 Практика](./exercises/chapter-15.md)

---

# Часть V. Производительность

## Глава 16. JavaScript и Rendering Pipeline

Темы:

- Parse
- Compile
- Execute
- Layout
- Paint
- Composite

* [📖 Читать главу](./book/chapter-16.md)  
* [📚 Литература](./references/chapter-16.md)  
* [💻 Примеры](./examples/chapter-16.md)  
* [🧪 Практика](./exercises/chapter-16.md)

---

## Глава 17. Современная загрузка

Темы:

- preload
- prefetch
- modulepreload
- fetchpriority
- lazy loading
- Priority Hints

* [📖 Читать главу](./book/chapter-17.md)  
* [📚 Литература](./references/chapter-17.md)  
* [💻 Примеры](./examples/chapter-17.md)  
* [🧪 Практика](./exercises/chapter-17.md)

---

## Глава 18. Off Main Thread

Темы:

- Web Workers
- Shared Workers
- Service Workers
- Worklets

* [📖 Читать главу](./book/chapter-18.md)  
* [📚 Литература](./references/chapter-18.md)  
* [💻 Примеры](./examples/chapter-18.md)  
* [🧪 Практика](./exercises/chapter-18.md)

---

## Глава 19. Современная память

Темы:

- WeakMap
- WeakSet
- WeakRef
- FinalizationRegistry

* [📖 Читать главу](./book/chapter-19.md)  
* [📚 Литература](./references/chapter-19.md)  
* [💻 Примеры](./examples/chapter-19.md)  
* [🧪 Практика](./exercises/chapter-19.md)

---

# Часть VI. Современная архитектура

## Глава 20. State Management без библиотек

Темы:

- Observable Pattern
- Signals
- EventTarget
- Custom Events
- URL как источник состояния
- History API

* [📖 Читать главу](./book/chapter-20.md)  
* [📚 Литература](./references/chapter-20.md)  
* [💻 Примеры](./examples/chapter-20.md)  
* [🧪 Практика](./exercises/chapter-20.md)

---

## Глава 21. Современные Web Components

Темы:

- Shadow DOM
- Declarative Shadow DOM
- Scoped Registries
- Declarative Custom Elements (перспективы)

* [📖 Читать главу](./book/chapter-21.md)  
* [📚 Литература](./references/chapter-21.md)  
* [💻 Примеры](./examples/chapter-21.md)  
* [🧪 Практика](./exercises/chapter-21.md)

---

## Глава 22. JavaScript и HTML Living Standard

Темы:

- HTML как API
- DOM как API
- CSS как API
- Почему JavaScript становится меньше

* [📖 Читать главу](./book/chapter-22.md)  
* [📚 Литература](./references/chapter-22.md)  
* [💻 Примеры](./examples/chapter-22.md)  
* [🧪 Практика](./exercises/chapter-22.md)

---

## Глава 23. Baseline и современная Web Platform

Темы:

- Что можно использовать без полифилов
- Progressive Enhancement
- Feature Detection
- Почему Baseline изменил архитектуру фронтенда

* [📖 Читать главу](./book/chapter-23.md)  
* [📚 Литература](./references/chapter-23.md)  
* [💻 Примеры](./examples/chapter-23.md)  
* [🧪 Практика](./exercises/chapter-23.md)

---

# Часть VII. JavaScript и современные фреймворки

## Глава 24. React после React 19

Темы:

- Compiler
- Server Components
- Actions
- Suspense

* [📖 Читать главу](./book/chapter-24.md)  
* [📚 Литература](./references/chapter-24.md)  
* [💻 Примеры](./examples/chapter-24.md)  
* [🧪 Практика](./exercises/chapter-24.md)

---

## Глава 25. Angular 2026

Темы:

- Signals
- Zoneless
- SSR
- Hydration
- Deferrable Views

* [📖 Читать главу](./book/chapter-25.md)  
* [📚 Литература](./references/chapter-25.md)  
* [💻 Примеры](./examples/chapter-25.md)  
* [🧪 Практика](./exercises/chapter-25.md)

---

## Глава 26. Vue 3.6+

Темы:

- Composition API
- Reactivity
- Vapor Mode

* [📖 Читать главу](./book/chapter-26.md)  
* [📚 Литература](./references/chapter-26.md)  
* [💻 Примеры](./examples/chapter-26.md)  
* [🧪 Практика](./exercises/chapter-26.md)

---

## Глава 27. Svelte 5

Темы:

- Runes
- Компилятор
- Минимум runtime

* [📖 Читать главу](./book/chapter-27.md)  
* [📚 Литература](./references/chapter-27.md)  
* [💻 Примеры](./examples/chapter-27.md)  
* [🧪 Практика](./exercises/chapter-27.md)

---

## Глава 28. Qwik

Темы:

- Resumability
- Serialization
- Lazy Execution

* [📖 Читать главу](./book/chapter-28.md)  
* [📚 Литература](./references/chapter-28.md)  
* [💻 Примеры](./examples/chapter-28.md)  
* [🧪 Практика](./exercises/chapter-28.md)

---

## Глава 29. Astro

Темы:

- Islands Architecture
- Partial Hydration
- Server-first

* [📖 Читать главу](./book/chapter-29.md)  
* [📚 Литература](./references/chapter-29.md)  
* [💻 Примеры](./examples/chapter-29.md)  
* [🧪 Практика](./exercises/chapter-29.md)

---

# Часть VIII. JavaScript и искусственный интеллект

## Глава 30. JavaScript для AI

Темы:

- WebGPU
- ONNX Runtime Web
- Transformers.js
- WebNN API
- Local AI

* [📖 Читать главу](./book/chapter-30.md)  
* [📚 Литература](./references/chapter-30.md)  
* [💻 Примеры](./examples/chapter-30.md)  
* [🧪 Практика](./exercises/chapter-30.md)

---

## Глава 31. Агентные приложения

Темы:

- MCP
- A2A
- AI SDK
- Streaming UI
- Function Calling

* [📖 Читать главу](./book/chapter-31.md)  
* [📚 Литература](./references/chapter-31.md)  
* [💻 Примеры](./examples/chapter-31.md)  
* [🧪 Практика](./exercises/chapter-31.md)

---

## Глава 32. JavaScript и WebAssembly

Темы:

- WASM
- Component Model
- WASI
- JavaScript ↔ Rust
- JavaScript ↔ C++

* [📖 Читать главу](./book/chapter-32.md)  
* [📚 Литература](./references/chapter-32.md)  
* [💻 Примеры](./examples/chapter-32.md)  
* [🧪 Практика](./exercises/chapter-32.md)

---

# Часть IX. Будущее JavaScript

## Глава 33. Новые предложения TC39

Темы:

- Records & Tuples
- Temporal
- Iterator Helpers
- Pipeline Operator
- Pattern Matching
- Explicit Resource Management

* [📖 Читать главу](./book/chapter-33.md)  
* [📚 Литература](./references/chapter-33.md)  
* [💻 Примеры](./examples/chapter-33.md)  
* [🧪 Практика](./exercises/chapter-33.md)

---

## Глава 34. JavaScript как язык Web Platform

Темы:

- HTML становится умнее
- CSS становится умнее
- Браузер становится умнее
- JavaScript становится меньше

* [📖 Читать главу](./book/chapter-34.md)  
* [📚 Литература](./references/chapter-34.md)  
* [💻 Примеры](./examples/chapter-34.md)  
* [🧪 Практика](./exercises/chapter-34.md)

---

## Глава 35. JavaScript 2030

Темы:

- Каким станет язык через несколько лет
- Роль AI
- Роль браузера
- Роль Web Platform
- Нужно ли будет писать столько JavaScript?

* [📖 Читать главу](./book/chapter-35.md)  
* [📚 Литература](./references/chapter-35.md)  
* [💻 Примеры](./examples/chapter-35.md)  
* [🧪 Практика](./exercises/chapter-35.md)

---

## 🛠️ Как читать книгу

Книга состоит из девяти частей и 35 глав. Рекомендуется читать последовательно, но вы также можете выбирать интересующие вас разделы.

Каждая глава содержит:

- теоретический материал в формате Markdown;
- ссылки на дополнительные источники;
- практические примеры кода;
- упражнения для закрепления знаний.

---

## 📦 Структура репозитория

```
modern-javascript-2026/
├── README.md
├── book/
│   ├── preface.md
│   ├── chapter-01.md
│   ├── chapter-02.md
│   └── ...
├── references/
│   ├── preface.md
│   ├── chapter-01.md
│   └── ...
├── examples/
│   ├── preface/
│   ├── chapter-01/
│   └── ...
└── exercises/
    ├── preface/
    ├── chapter-01/
    └── ...
```

---

## ✍️ Автор

Книга создана для сообщества разработчиков, стремящихся углубить свои знания о современном JavaScript и Web Platform.

---

## 📄 Лицензия

Книга распространяется под лицензией [Creative Commons Attribution 4.0 International](https://creativecommons.org/licenses/by/4.0/).

---

## 🙏 Благодарности

Особая благодарность сообществу TC39, разработчикам браузеров и всем, кто делает Web Platform лучше каждый день.

